...
import {
  TasksService, ActivitiesService,
  type TaskPublic, type ActivityPublic,
  type TaskCreate, type ActivityCreate,
} from "@/client"
...
function AddTask() {
  ...
  const mut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["tasks"] }); showSuccessToast("Task added."); reset(); setOpen(false) },
  })

  const onSubmit = (d: any) => mut.mutate({
    title: d.title, deal_name: d.deal_name || undefined,
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          <div className="space-y-1.5">
            <Label>Task *</Label>
            <Input {...register("title", { required: true })} placeholder="e.g. Follow up with Sun Group on commercial terms" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label>Related Deal</Label>
              <Input {...register("deal_name")} placeholder="Deal name" />
            </div>
            <div className="space-y-1.5">
...
function LogActivity() {
  ...
  const mut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["activities"] }); showSuccessToast("Activity logged."); reset({ type_s: "Meeting", date: today }); setOpen(false) },
  })

  const onSubmit = (d: any) => mut.mutate({
    activity_type: d.type_s, date: d.date,
    deal_name: d.deal_name || undefined, note: d.note || undefined,
  })
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div className="space-y-1.5">
            <Label>Related Deal</Label>
            <Input {...register("deal_name")} placeholder="Deal name (optional)" />
          </div>
          <div className="space-y-1.5">
            <Label>Note</Label>
...
function TaskRow({ task }: { task: TaskPublic }) {
  ...
  const markDone = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["tasks"] }); showSuccessToast("Task marked done.") },
  })
  ...
  return (
    <tr className="border-b last:border-0 hover:bg-muted/30 transition-colors">
      ...
      <td className="py-2.5 pr-3">
        <span className="text-sm">{task.task_owner ?? "—"}</span>
      </td>
      <td className="py-2.5 pr-3">
        <span className={`text-xs font-medium ${task.is_overdue ? "text-red-600 font-semibold" : "text-muted-foreground"}`}>
          {task.due_date ?? "—"}
        </span>
      </td>
      <td className="py-2.5 pr-3">
      ...
      <td className="py-2.5 pr-2">
        <div className="flex gap-0.5">
          {task.status !== "Done" && (
            <Button variant="ghost" size="sm" className="h-7 px-2 text-green-600 hover:text-green-700" onClick={() => markDone.mutate()}>
              <CheckSquare className="h-3.5 w-3.5" />
            </Button>
          )}
          <Button variant="ghost" size="sm" className="h-7 px-2 text-red-500 hover:text-red-600" onClick={() => del.mutate()}>
      ...
      <div className="grid grid-cols-[1fr_360px] gap-6">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold">Open Tasks <span className="text-muted-foreground font-normal text-sm">({tasks.length})</span></h2>
                ...
                <thead>
                  <tr className="border-b bg-muted/30">
                    {["Task","Owner","Due","Priority","Status",""].map(h => (
                      <th key={h} className="text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground py-2.5 pr-3 pl-2 first:pl-2">
                        {h}
                      </th>
                    ))}
                  </tr>
          ...
          <div className="rounded-lg border bg-card p-4 overflow-y-auto max-h-[600px]">
            {activities.length === 0 ? (
              ...
              <div className="relative pl-4 border-l-2 border-muted flex flex-col gap-4">
                {activities.map((a: ActivityPublic) => (
                  <div key={a.id} className="relative">
                    <div className="absolute -left-[21px] top-1 h-2.5 w-2.5 rounded-full bg-primary border-2 border-background" />
                    <p className="text-xs font-semibold">{a.activity_type}
                      <span className="text-muted-foreground font-normal ml-2">{a.date}</span>
                    </p>
                    {a.deal_name && <p className="text-xs text-muted-foreground">{a.deal_name}</p>}
                    {a.note && <p className="text-xs text-foreground/70 mt-0.5">{a.note}</p>}
                  </div>
                ))}
              </div>
            )}
