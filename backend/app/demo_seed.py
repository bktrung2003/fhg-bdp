"""Demo seed data + loaders — importable from API routes and CLI script.

Creates rich realistic demo data:
  - 6 owners (hospitality developers across APAC)
  - 8 projects (Vietnam destinations + 1 Thailand)
  ...
  - 20 tasks (mix of open/in-progress/done)
  - 26 activities (last 60 days)
  - 13 pre-opening milestones

All records tagged via DEMO_SEED note/tag for safe re-runs. The clear_demo()
function removes them by name match.
"""
import json
import uuid
...
SEED_TAG = "DEMO_SEED"
# Refreshed inside load_demo() so re-runs get current timestamps.
...
# NOTE: Strict enums — use ONLY these:
#   relationship    : New | Warm | Strong | Strategic Partner | Risk / Unstable
#   catchup_status  : On track | Due this week | Overdue | No cadence
#   contact strength: New | Warm | Strong
OWNERS = [
    {"company": "Sun Group", "country": "Vietnam", "priority": "Strategic",
     "relationship": "Strategic Partner", "catchup_status": "On track",
    ...
    {"company": "Vingroup", "country": "Vietnam", "priority": "Medium",
     ...
     "contacts": [
         {"fusion_role": "BD Director", "owner_contact": "Mr. Phạm Văn Hùng (Investment Director)",
          "strength": "Warm", "senior_flag": False,
    ...
    {"company": "FLC Group", "country": "Vietnam", "priority": "Low",
     ...
     "contacts": [
         {"fusion_role": "BD Manager", "owner_contact": "Ms. Trần Thị Mai (Acting CEO)",
          "strength": "New", "senior_flag": False,
    ...
    {"company": "NovaLand", "country": "Vietnam", "priority": "Medium",
     ...
     "contacts": [
         {"fusion_role": "BD Director", "owner_contact": "Mr. Bùi Thành Nhơn (Founder)",
          "strength": "Warm", "senior_flag": True,
...
# NOTE: ActivityType enum — Meeting | Dinner | Site visit | Phone call | WhatsApp summary
...
ACTIVITIES = [
    ...
    {"deal": "Fusion Phú Quốc HMA", "type": "Other",            "days_ago": 5,  "note": "Email: Sent executed HMA copy to Legal team archive."},
    {"deal": "Fusion Phú Quốc HMA", "type": "Phone call",       "days_ago": 10, "note": "Mr. Tuấn called to confirm signing ceremony details."},
    {"deal": "Fusion Phú Quốc HMA", "type": "HMA signed",       "days_ago": 30, "note": "HMA executed — final version uploaded to Documents."},
    {"deal": "BRG Hanoi Downtown HMA", "type": "Meeting",       "days_ago": 1,  "note": "Mr. Hai term sheet review. Pushing back on 2.25% base fee."},
    {"deal": "BRG Hanoi Downtown HMA", "type": "Other",         "days_ago": 3,  "note": "Email: Sent revised LOI draft to BRG legal team."},
    {"deal": "BRG Hanoi Downtown HMA", "type": "Meeting",       "days_ago": 7,  "note": "Competitive intelligence: Marriott pitched 1.8% base + IMF tiered."},
    {"deal": "BRG Hanoi Downtown HMA", "type": "Phone call",    "days_ago": 14, "note": "Initial term sheet alignment call. Gaps identified on fee + term."},
    {"deal": "Vinpearl Marina FA",     "type": "LOI signed",    "days_ago": 7,  "note": "LOI executed."},
    ...
    {"deal": "Sun World Đà Nẵng HMA", "type": "Proposal sent",  "days_ago": 14, "note": "Submitted initial proposal with standard fee structure."},
    {"deal": "Sun World Đà Nẵng HMA", "type": "Site visit",     "days_ago": 28, "note": "Site walkthrough. Identified BOH design gaps."},
    {"deal": "Sun World Đà Nẵng TSA", "type": "Other",          "days_ago": 3,  "note": "Email: TSA scope clarification sent."},
    {"deal": "NovaWorld Phan Thiết HMA", "type": "Meeting",     "days_ago": 8,  "note": "Mr. Nhơn meeting. Discussed financing timing."},
    {"deal": "NovaWorld Phan Thiết HMA", "type": "Other",       "days_ago": 21, "note": "Document: NovaLand financial statements reviewed — bridge financing concern."},
    {"deal": "NovaWorld Phan Thiết HMA", "type": "Site visit",  "days_ago": 35, "note": "Site visit to Phan Thiết master plan area."},
    {"deal": "Sầm Sơn Resort HMA", "type": "Other",             "days_ago": 60, "note": "Deal marked Lost — FLC regulatory unresolved 18+ months."},
    {"deal": "Phuket Patong HMA", "type": "Meeting",            "days_ago": 7,  "note": "Initial intro call with Mr. Chatchawal. Site visit scheduled."},
    {"deal": "Phuket Patong HMA", "type": "NDA signed",         "days_ago": 10, "note": "NDA executed and returned."},
    {"deal": "Fusion Đà Lạt HMA", "type": "Pre-opening review", "days_ago": 1,  "note": "Pre-opening 60-day countdown review."},
    {"deal": "Fusion Đà Lạt HMA", "type": "Other",              "days_ago": 5,  "note": "Document: Brand standards compliance audit complete — all green."},
    {"deal": "Fusion Đà Lạt HMA", "type": "Pre-opening review", "days_ago": 14, "note": "Weekly pre-opening standup. PMS on schedule."},
    {"deal": "Vinpearl Marina Pre-opening", "type": "Other",    "days_ago": 2,  "note": "Email: Pre-opening services scope draft sent."},
    {"deal": "Phú Quốc Villa Annex Lead", "type": "Phone call", "days_ago": 1,  "note": "Initial scoping call with Sun Group team about villa annex."},
    {"deal": "BRG Hải Phòng Sister Project", "type": "Other",   "days_ago": 180,"note": "Note: Hotel opened successfully. Soft launch complete."},
]
...
def seed_owners(session: Session, admin: User) -> dict[str, uuid.UUID]:
    ...
    for od in OWNERS:
        ...
        session.add(OwnerInteraction(
            owner_id=owner.id, interaction_type="Phone call",
...
def load_demo(session: Session, admin: User, *, verbose: bool = False) -> dict[str, int]:
    """Load all demo data. Clears existing demo records first (idempotent).
    Returns counts dict. Caller is responsible for committing."""
    global NOW
    NOW = datetime.now(timezone.utc)  # refresh timestamps for this run

    def _log(msg: str) -> None:
        if verbose: print(msg)

    _log("  Clearing previous demo data...")
    clear_demo(session)

    _log(f"  Seeding {len(OWNERS)} owners...")
    owner_map = seed_owners(session, admin)
    session.flush()

    _log(f"  Seeding {len(PROJECTS)} projects...")
    project_map = seed_projects(session, admin, owner_map)
    session.flush()

    _log(f"  Seeding {len(DEALS)} deals...")
    deal_map = seed_deals(session, admin, project_map, owner_map)
    session.flush()

    _log(f"  Seeding {len(ASSESSMENTS)} feasibility assessments...")
    seed_assessments(session, admin, deal_map)

    _log(f"  Seeding {len(SNAPSHOTS)} financial snapshots...")
    seed_snapshots(session, admin, deal_map)

    _log(f"  Seeding {len(TASKS)} tasks...")
    seed_tasks(session, admin, deal_map)

    _log(f"  Seeding {len(ACTIVITIES)} activities...")
    seed_activities(session, admin, deal_map)

    _log(f"  Seeding {len(MILESTONES)} milestones...")
    seed_milestones(session, admin, project_map)

    return {
        "owners": len(OWNERS), "projects": len(PROJECTS), "deals": len(DEALS),
        "assessments": len(ASSESSMENTS), "snapshots": len(SNAPSHOTS),
        "tasks": len(TASKS), "activities": len(ACTIVITIES),
        "milestones": len(MILESTONES),
    }


# Inventory used by Settings UI to show what's included
DEMO_SUMMARY = {
    "owners": len(OWNERS), "projects": len(PROJECTS), "deals": len(DEALS),
    "assessments": len(ASSESSMENTS), "snapshots": len(SNAPSHOTS),
    "tasks": len(TASKS), "activities": len(ACTIVITIES),
