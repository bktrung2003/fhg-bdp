...
def _to_public(doc: Document) -> DocumentPublic:
    url = get_download_url(doc.storage_path, doc.original_filename)
    return DocumentPublic(**doc.model_dump(), download_url=url)
...
def list_documents(
    ...
    return DocumentsPublic(data=[_to_public(d) for d in docs], count=count)
...
def serve_local_file(key: str, current_user: CurrentUser) -> Any:
